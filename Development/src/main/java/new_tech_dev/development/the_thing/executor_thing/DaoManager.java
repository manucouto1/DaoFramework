...
public class DaoManager {
	...
	/*
	 * Recupera informacion de los metodos del dao y del daoXml Crea un mapa de
	 * DaoMetodos con el nombre del metodo como clave
	 */
			...
			for (int i = 0; i < methodsList.length; i++) {
				this.daoMethods.put(methodsList[i].getName(),
						new DaoMethod(methodsList[i], querys.get(methodsList[i].getName()),
								...
								ReturnFactory.getReturnCaster(methodsList[i],
										getClassFromList(methodsList[i].getGenericReturnType(), classV, classK)),
	...
	/*
	 * Se le pasa el tipo devuelto por un metodo, si es una lista se devuelve el
	 * tipo parametrizado
	 */
	...
	private Type typeFromGeneric(Class<?> V, Class<?> K, Type type) {
		return (type.getTypeName().equalsIgnoreCase("V")) ? V : (type.getTypeName().equalsIgnoreCase("K")) ? K : type;
	}

	/*
	 * recupera el DaoMethod del mapa, se le pasan los argumentos y la conexion
	 */
